...
import { loadImage } from '@/engine/common/common';

export class SetRuler {
  constructor () {
    this.D_rx = document.getElementById('ruler-x');
    this.D_ry = document.getElementById('ruler-y');
    this.hMarkNumImg = null;
    this.vMarkNumImg = null;
  }

  updateSize () {
    const { cw, ch } = window.$vue.$store.getters.getCMsg;

    this.D_rx.setAttribute('width', cw);
    this.D_rx.setAttribute('height', 25);
    this.D_ry.setAttribute('width', 25);
    this.D_ry.setAttribute('height', ch);
  }
  ...
  async drawRulerTop (sp, ox, us) {
    ...
    for (let i = Math.floor(ox / sp); i >= 0; i--) {
      ...
      if (i % 10 === 0) {
        lh = 18;
        drawNumber(ctx, this.hMarkNumImg, -i * us, (ox - (i * sp)) + 2, ch - 18 + 2);
      }
  ...
  async drawRulerLeft (sp, oy, us) {
    const D_ruler = this.D_ry;
    const ctx = D_ruler.getContext('2d');
    const cw = D_ruler.clientWidth;
    const ch = D_ruler.clientHeight;

    if (!this.vMarkNumImg) {
      this.vMarkNumImg = await loadImage(imgsrc_l);
    }
    ...
    for (let i = Math.floor(oy / sp); i >= 0; i--) {
      ...
      if (i % 10 === 0) {
        ...
        drawNumber(ctx, this.vMarkNumImg, i * us, cw - 18 + 2, (oy - (i * sp)) + 2);
      }
